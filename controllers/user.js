...
exports.postCategory = (req, res, next) => {
   ...
   connectDB.query(data, (err, result) => {
      if (err) throw err; //show if error found
         ...
         //console.log(result);
         return res.render('user/showCategory', { user: req.session.mail, football: result })
      }
   })
...
exports.postStatus = (req, res, next) => {
   ...
   data = "INSERT INTO bookingstatus " +
      " VALUES ('" + req.session.mail + "','" + req.body.name + "','" + req.body.type + "','" + req.body.time + "','" + 0 + "','" + date + "')"
   ...
   connectDB.query(data, (err, reslt) => {
      if (err) throw err;
         ...
         connectDB.query(data1, (err1, result) => {
            ...
            res.render('user/statusShow', { user: req.session.mail, msg: "Đặt sân thành công", err: "", data: result });
         })
      }
   })
}
...
exports.getShowStatus = (req, res, next) => {
   ...
   connectDB.query(data, (err, result) => {
      ...
      if (err) throw err;
         ...
         if (result.length < 1) {
            res.render('user/statusShow', { user: req.session.mail, msg: "", err: "Không có dữ liệu", data: result });
         }
         else {
